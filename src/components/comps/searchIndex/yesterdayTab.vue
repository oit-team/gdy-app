<template>
  <div class="h-full">
    <div v-if="yesterdayResultList.length > 0" ref="yesterdayRef" class="max-h-110 flex pt-2 overflow-x-auto sellBox">
      <div class="sellItem bg-[#F9F9F9] relative p-1" v-for="(item,index) in yesterdayResultList" :key="index">
        <div>
...
export default {
  ...
  activated() {
    if(this.isClickToDetail){
      this.$refs.yesterdayRef.scrollIntoView({behavior:'smooth', block: "center"})
    }
  },
  methods: {
    convertImageSize,
