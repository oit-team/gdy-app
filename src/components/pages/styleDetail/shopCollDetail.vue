          ...
          <div v-if="commentList && commentList.length>0">
            <div v-for="(item,index) in commentList" :key="index" class="comCon">
              ...
              <div class="avatarBox">
                ...
                <span class="praise">{{item.pjTime}}</span>
              </div>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      commentList:[],
      aeFlag:0,  // 0 未点赞  1 已点赞
      aeNumber:0,   // 点赞数量
      collStatus:null,  // 新增搭配的状态  0 已保存 1 审核中 2 审批拒绝 3 已发布

      approvalList:[],   // 搭配被拒绝理由列表
    };
  },
  created(){
    // console.log("this.$route.query",this.$route.query);
    this.collStatus = this.$route.query.collStatus;
    if(this.$route.query.collInfo){
      this.approvalList = this.$route.query.collInfo.approvalList;
    }
  ...
  methods:{
    ...
    collPraise(){
      ...
      if(_this.aeFlag){
        ...
        _this.$axios.post("/api/coll/addPutApt",params).then(function(res){
          let data = res.data;
          // console.log("搭配点赞",data)
          if(data.code=="200"){
            //  _this.$toast("点赞成功")
            // aeFlag: 1
            // aeNumber: 1
            if(!_this.aeFlag){
              _this.aeFlag = !_this.aeFlag;
              _this.aeNumber  = _this.aeNumber + 1;
            }
    ...
    getImg(index){
      ...
      // console.log(`点击第${index+1}张图片预览进行`,index+1);
    ...
    toShopCollComment(){
      this.$router.push({path:'/shopCollComment',query:{collId:this.$route.query.collId}})
    },
    ...
    getStyleInfo(id){
      ...
      .then(function(res){
        ...
        if(data.code=="200"){
          ...
          _this.aeNumber = _this.collInfo.aeNumber;
          _this.aeFlag = _this.collInfo.aeFlag;
